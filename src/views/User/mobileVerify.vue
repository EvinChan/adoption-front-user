                            ...
                            <div class="assets">
                                ...
                                <div style="padding: 20px 215px 0 215px;">
                                    <el-form ref="mobileForm"
                                             ...
                                             :rules="mobileRules"
                                             label-width="100px">
                                        <el-form-item label="原账号" prop="oldMobile">
                                            <el-input v-model="mobileForm.oldMobile"
                                                      placeholder="请输入手机号"
                                                      auto-complete="off"
                                                      maxlength="11"/>
                                        </el-form-item>
...
<script>
  ...
  import {checkOldMobile, getUserSms, checkCode} from '@/api/user';
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        mobileRules: {
          oldMobile: [{required: true, trigger: 'blur', validator: validateMobile}],
    ...
    methods: {
      sendVerification() {
        if (!validMobile(this.mobileForm.oldMobile)) {
          this.$message.error("请输入正确的手机号")
          return false
        } else {
          checkOldMobile(this.mobileForm.oldMobile).then(result => {
            if (result.code === 200) {
              getUserSms(this.mobileForm.oldMobile).then(result => {
                if (result.code === 200) {
      ...
      handleUpdate() {
        this.$refs.mobileForm.validate(valid => {
          if (valid) {
            checkCode(this.mobileForm.oldMobile, this.mobileForm.verificationCode).then(result => {
              if (result.code === 200) {
                this.$router.push({path: '/mobile/set'})
              } else {
                return false
              }
            })
